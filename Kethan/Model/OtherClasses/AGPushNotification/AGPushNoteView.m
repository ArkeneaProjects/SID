...
- (void)setUpUI {
    ...
    self.frame = CGRectMake(f.origin.x,0, width, 80);
...
+ (void)showWithNotificationMessage: (NSString *)message withBody: (NSString *)body {
    PUSH_VIEW.bodyLabel.text = body;
...
+ (void)showWithNotificationMessage: (NSString *)message completion: (void (^)(void))completion {
    ...
    if (message) {
        ...
        PUSH_VIEW.appTitle.text = appName;
        CGRect f = PUSH_VIEW.frame;
        NSLog(@"print==\%f",APP.window.frame.size.height);
        ...
        [UIView animateWithDuration: SHOW_ANIM_DUR delay: 0 options: UIViewAnimationOptionCurveEaseOut animations:^{
            CGRect f = PUSH_VIEW.frame;
            CGFloat y = (APP.window.frame.size.height == 812 || APP.window.frame.size.height == 736) ?20.0:0;
            PUSH_VIEW.frame = CGRectMake(f.origin.x, y, f.size.width, f.size.height);
