...
static CGFloat const KVNContentViewWithStatusInset = 20.0f;
...
static CGFloat const KVNAlertViewWidth = 320.0f;
		  ...
		  completion: (KVNCompletionBlock)completion
{
	...
	if ([self.class isVisible]) {
		...
		dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
			if (![KVNBlockSelf.queue.operations containsObject: __blockSelf.animateAppearanceOperation] &&
	...
	if (self.style != KVNProgressStyleProgress) {
        ...
        CGFloat lines = floor(self.statusLabel.bounds.size.height/20.0);
        if (lines <= 0) {
            lines = 1;
        }
		...
		switch (self.style) {
			...
			case KVNProgressStyleSuccess:
                delay = self.configuration.minimumSuccessDisplayTime * lines;
				break;
			case KVNProgressStyleError:
                delay = self.configuration.minimumErrorDisplayTime * lines;
				break;
...
- (void)setupErrorUI
{
    ...
    [crossPath moveToPoint: CGPointMake(CGRectGetWidth(self.circleProgressView.bounds) * 0.5f, CGRectGetHeight(self.circleProgressView.bounds) * 0.05f)];
    [crossPath addLineToPoint: CGPointMake(CGRectGetWidth(self.circleProgressView.bounds) * 0.1f, CGRectGetHeight(self.circleProgressView.bounds) * 0.85f)];
    [crossPath addLineToPoint: CGPointMake(CGRectGetWidth(self.circleProgressView.bounds) * 0.9f, CGRectGetHeight(self.circleProgressView.bounds) * 0.85f)];
    [crossPath addLineToPoint: CGPointMake(CGRectGetWidth(self.circleProgressView.bounds) * 0.5f, CGRectGetHeight(self.circleProgressView.bounds) * 0.05f)];
    
    [crossPath moveToPoint: CGPointMake(CGRectGetWidth(self.circleProgressView.bounds) * 0.5f, CGRectGetHeight(self.circleProgressView.bounds) * 0.35f)];
    [crossPath addLineToPoint: CGPointMake(CGRectGetWidth(self.circleProgressView.bounds) * 0.5f, CGRectGetHeight(self.circleProgressView.bounds) * 0.6f)];
    
    [crossPath moveToPoint: CGPointMake(CGRectGetWidth(self.circleProgressView.bounds) * 0.5f, CGRectGetHeight(self.circleProgressView.bounds) * 0.7f)];
    [crossPath addArcWithCenter: CGPointMake(CGRectGetWidth(self.circleProgressView.bounds) * 0.5f, CGRectGetHeight(self.circleProgressView.bounds) * 0.7f) radius:0.5 startAngle:0 endAngle: M_PI * 2 clockwise: YES];
    
	crossPath.lineCapStyle = kCGLineCapRound;
    crossPath.lineJoinStyle = kCGLineJoinRound;
	
    [crossPath applyTransform: CGAffineTransformMakeTranslation(0, 8.0)];
    
    self.circleProgressView.layer.cornerRadius = 0;
    self.circleProgressView.clipsToBounds = NO;
    self.circleProgressView.layer.masksToBounds = NO;
